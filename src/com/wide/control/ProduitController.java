...
public class ProduitController implements Serializable {
	...
	private List<Produit> itemsCommande = new ArrayList<Produit>();
	...
	public List<Produit> getItemsCommande() {
		return itemsCommande;
	}

	public void setItemsCommande(List<Produit> itemsCommande) {
...

	{
		...
		for (Magasin magasin : list) {
			ok = true;
			for (Magasinproduit magasinproduit : magasin
					.getMagasinproduitCollection()) {
				if (magasinproduit.getRefProd().getRefProd() == selected
						.getRefProd()) {
					ok = false;
					break;
				}
	...
	public ProduitController() {
		...
		for (Produit produit : produits1) {
			for (Magasinproduit magasinproduit : magasinproduits) {
				...
				if (produit.getRefProd() == magasinproduit.getRefProd()
						.getRefProd()) {
					ok = false;
					break;
				}
	...
	public void createApprovisionnement() {
		...
		qte = 0;
		msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "Demande Envoyé",
				"Votre demande d'approvisionnement est envoyé avec succes au chef depot");
...
//		qte=selected.getQuantite();
	...
	public void updateCommande() {
		...
		LigneCommandeFacade ligneCommandeFacade = new LigneCommandeFacade();
		ProduitFacade produitFacade = new ProduitFacade();
		Commande commande = new Commande();
		commande.setValidee(false);
		
		commandeFacade.create(commande);

			LigneCommande ligneCommande = new LigneCommande();
			ligneCommande.setIdCommande(commande);
			ligneCommande.setRefProd(selected);
			ligneCommande.setQteCom(qteCommande);
			ligneCommandeFacade.create(ligneCommande);
			
			msg=new FacesMessage(FacesMessage.SEVERITY_INFO, "Demande Envoyé", "Votre demande de commande est envoyé");
			 FacesContext.getCurrentInstance().addMessage(null, msg);
	         context.addCallbackParam("loggedIn", false);
	...
	public void confirmerCommande() {
		...
		for (Produit produit : itemsCommande) {
			LigneCommande ligneCommande = new LigneCommande();
			ligneCommande.setIdCommande(commande);
			ligneCommande.setRefProd(produit);
			ligneCommandeFacade.create(ligneCommande);
			System.out.println("Ligne commande sssssssssssssss");
		}

		itemsCommande = new ArrayList<Produit>();
		produits.setWrappedData(itemsCommande);
	}
	...
	public List<Produit> getItems() {
		...
		for (Produit produit : produits) {
			boolean ok=false;
			for (LigneCommande ligneCommande : produit
					.getLigneCommandeCollection()) {
				if (ligneCommande.getIdCommande().getValidee()) {
					ok=true;
					produit.setQuantite(produit.getQuantite()+ligneCommande.getQteCom());
					
					ligneCommande.setQteCom(0);
					ligneCommandeFacade.edit(ligneCommande);
				}
			}
			if(ok){
				ejbFacade.create(produit);
				items.add(produit);
			}
		}
		return items;
	...
	public static class ProduitControllerConverter implements Converter {
		...
		public Object getAsObject(FacesContext facesContext,
			...
			ProduitController controller = (ProduitController) facesContext
					.getApplication()
					.getELResolver()
